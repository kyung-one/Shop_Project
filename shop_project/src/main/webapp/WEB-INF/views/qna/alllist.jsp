...
<div class="container mt-3">
	...
	<table class="table table-hover">
		...
		<tbody>
			<c:forEach items="${qalllist.content }" var="lists" varStatus="st">
				<tr>
					<td class="text-center">${lists.qnanum}</td>
					<td class="text-center">${lists.product.productid }</td>
	...
	<div class="d-flex justify-content-between">
		...
		<ul class="pagination mt-3 ">
			...
			<!-- 이전  -->
			<c:if test="${qalllist.first==false}">
				<li class="page-item"><a class="page-link"
					href="?page=${qalllist.number-1 }&field=${param.field}&word=${param.word}">이전</a></li>
			</c:if>

			<!-- 다음  -->
			<c:if test="${qalllist.last==false}">
				<li class="page-item"><a class="page-link"
					href="?page=${qalllist.number+1 }&field=${param.field}&word=${param.word}">다음</a></li>
			</c:if>
